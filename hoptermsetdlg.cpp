...
#include <QTextStream>
#include <QDir>
HOpTermSetDlg::HOpTermSetDlg(QWidget *parent) :
    QDialog(parent),
...
HOpTermSetDlg::~HOpTermSetDlg()
{
    delete ui;
}

void HOpTermSetDlg::initDlg()
{
    loadOpTerm();
...
void HOpTermSetDlg::refreshTable(int currentIndex)
{
...
void HOpTermSetDlg::tableItemClicked(QTableWidgetItem *item)
{
    if(item == NULL) return;
...
void HOpTermSetDlg::modifyOpTerm()
{
...
void HOpTermSetDlg::loadOpTerm()
{
    ...
    strcat(szPath,"/default_optermitem_config.ini");
...
void HOpTermSetDlg::saveOpTerm()
{
     ...
     strPath += "/default_optermitem_config.ini";
     QFile file(strPath);
     if(file.exists())
     {
...
void HOpTermSetDlg::closeOpTerm()
{
    if(bModifyed)
    {
