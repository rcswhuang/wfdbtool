#ifndef GLOSSARYDLG_H
#define GLOSSARYDLG_H
...
typedef struct _tagtreeGlossary
{
    uchar  btTreeType;//类型：树形结构，开关刀闸信号缺省，术语，分术语合术语提示术语
    uchar  btGlossaryType;//开关
    ushort wGroupID;//对应组ID 如果是操作术语为组ID，如果是某个术语则是所在操作术语的ID
    ushort wGlossaryID;//如果是是操作术语则是空，某个术语则是其ID

}TREEGLOSSARY;
Q_DECLARE_METATYPE(TREEGLOSSARY*)
...
class HGlossaryDlg : public QDialog
{
    Q_OBJECT

public:
    explicit HGlossaryDlg(QWidget *parent = 0);
    ~HGlossaryDlg();
public:
    void initDlg();
...
public slots:
    ...
    void addDZGlossary();
    void addXHGlossary();
    void delGlossary();//删除一组操作术语
    void addTSGlossary();//增加某组操作术语中的提示术语
    void delTSGlossary();//删除某组操作术语中的提示术语
